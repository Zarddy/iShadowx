...
public class CnNumberUtils {
    ...
    // 万元以上高单位
    private final static String[] CN_NUM_HIGH_UNIT = {"", "万", "亿", "万亿"};
    // 小数位单位
    ...
    public static String toUppercase(Double number) {
        return toUppercase(String.valueOf(number));
    }

    public static String toUppercase(float number) {
        return toUppercase(String.valueOf(number));
    }
    ...
    public static String toUppercase(String num){
        ...
        for(int i=0; i<intLength; i++){
            ...
            if(!isZero(curCnNum)){
                ...
                String heightUnit = CN_NUM_HIGH_UNIT[hui];
                //如果剩余没有低金额单位或者后续都为零则加上高单位
                if(ui == 0 || isZeroInResidue(chars, i, ui)){
                    strBuilder.append(heightUnit);
                    //万单位以后的千单位如果不能为零则跳过零
                    if(heightUnit.equals(CN_NUM_HIGH_UNIT[1]) && !isZeroOfIndex(chars, (ui+1)+i)){
